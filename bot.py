...
async def verify_captcha(event, user_id):
    ...
    # Check if captcha already sent
    if user[19] and user[20]:
        try:
            expiry = datetime.strptime(user[20], '%Y-%m-%d %H:%M:%S')
        ...
        if task_id in completed_tasks:
            ...
            if task_type == 'join':
                try:
                    ...
                    buttons.append([Button.inline("✅ Check & Verify", f"verify_task_{task_id}")])
                except:
                    msg += f"   🔗 Invalid link\n\n"
                    buttons.append([Button.inline("✅ Check & Verify", f"verify_task_{task_id}")])
...
async def handle_captcha(event):
    ...
    # Only process if user is not verified and has captcha code
    if len(user) > 19 and user[11] == 0 and user[19]:
        # Check if message is a number (captcha answer)
        if event.raw_text.strip().isdigit():
            try:
...
async def start(event):
    ...
    if user[11] == 0:
        # Check if captcha already sent
        if not (user[19] and user[20]):
            await verify_captcha(event, user_id)
        else:
            await event.reply("🔐 *Verification required!*\n\nPlease answer the captcha question sent earlier.\n\nSend the answer to continue.")
    else:
        await main_menu(event, user_id)

print("=" * 60)
print("🤖 TASK MASTER PRO BOT STARTED!")
print("=" * 60)
